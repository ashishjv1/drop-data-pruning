...
# Disable APScheduler logging completely
logging.getLogger('apscheduler').setLevel(logging.ERROR)
logging.getLogger('apscheduler.executors.default').setLevel(logging.ERROR)
logging.getLogger('apscheduler.scheduler').setLevel(logging.ERROR)

class EcoLogFilter(logging.Filter):
    def filter(self, record):
        # Filter out logs containing eco2ai related content or from APScheduler
        excluded_terms = [
            'eco2ai', 'Tracker', 'emissions', 'CO2',
            'consumption', 'power', 'RAM', 'CPU', 'GPU',
            'memory', 'usage', 'total', 'apscheduler'
        ]
        # Check both the logger name and message content
        return not (any(x.lower() in record.msg.lower() for x in excluded_terms) or 
                   'apscheduler' in record.name.lower())

# Configure eco2ai tracker with proper data type handling
def configure_eco2ai():
    # Ensure APScheduler logging is disabled
    ...
    ttype = "train"  # Since this is training
    tracker_obj = Tracker(
        project_name=f"{args.model_name}_dset-{args.dataset_name}_bs-{args.batch_size}",
        experiment_description=f"Training with data pruning strategy={args.strategy}",
        file_name=f"emissions_{args.model_name}_dset-{args.dataset_name}_bs-{args.batch_size}_epochs-{args.epochs_final}_frac-{args.final_frac}_SGD_{ttype}.csv"
    )
    
    # Ensure the DataFrame has proper data types from the start
    if not os.path.exists("emissions.csv"):
        df = pd.DataFrame(columns=[
            'timestamp', 'project_name', 'experiment_description', 
            'duration', 'power_consumption', 'CO2_emissions', 
            'CPU_usage', 'RAM_usage', 'RAM_total', 'GPU_usage', 
            'GPU_memory_usage', 'GPU_memory_total'
        ])
        df = df.astype({
            'timestamp': str,
            'project_name': str,
...
parser.add_argument('--scheduler_type', type=str, default='multistep', help='learning rate scheduler type: multistep or cosine')
...
def main(args):
    # Configure & initialize eco2ai
    emission_tracker = configure_eco2ai()
    ...
    eco_filter = EcoLogFilter()
    for name in logging.root.manager.loggerDict:
        if logging.getLogger(name).handlers:
            logging.getLogger(name).addFilter(eco_filter)
    
    # Get logger for this module and add filter
    logger = logging.getLogger(__name__)
    logger.addFilter(eco_filter)
